 ...
 */
function isMember() {
    return isset($_SESSION['user_id'], $_SESSION['user_role']) && strtolower($_SESSION['user_role']) === 'member';
}

/**
 * Check if user can manage shared member documents
 */
function canManageMemberDocuments() {
...
/**
 * Security middleware: Member document access
 */
...
/**
 * Security middleware: document access for editors or members
 */
function requireDocumentAccess() {
    if (!isMember() && !canManageMemberDocuments() && !canEdit()) {
        ob_clean();
        http_response_code(403);
        exit('Access Denied.');
    }
...
if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT nu.user_id, nu.username, nu.full_name, nu.email, nu.password_hash, nu.must_change_password, nu.nbbtm_role_id,
                     COALESCE(NULLIF(nr.nbbtm_role_name, ''), nu.role) AS effective_role, nu.is_active
              FROM nbbtm_users nu
    ...
    if ($user = $result->fetch_assoc()) {
        ...
        if (password_verify($password, $user['password_hash'])) {
            ...
            $_SESSION['user_role']  = strtolower($user['effective_role'] ?? 'browser');
            $_SESSION['must_change_password'] = (int)$user['must_change_password'] === 1;
            if ($_SESSION['user_role'] === 'view') {
                $_SESSION['user_role'] = 'browser';
            }
...
if ($action === 'change_password' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user = $result->fetch_assoc()) {
        ...
        $updateStmt = $db->prepare("UPDATE nbbtm_users SET password_hash = ?, must_change_password = 0 WHERE user_id = ?");
...
if ($action === 'logout') {
    ...
    // Explicitly clear user session data and force browser role
