            ...
            function loadReferenceData() {
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) contactsCache = data.contacts || [];
                    }
                });
                $.ajax({
                    url: 'prg_event_api.php',
                    type: 'GET',
                    data: {
                        action: 'get_ministries_list'
                    },
                    dataType: 'json',
                    success: function(data) {
                        if (data.success) ministriesCache = data.ministries || [];
                    }
                });
            }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success && data.prgevnt) {
                            ...
                            // Registration & Fee (section hidden entirely if registration not required)
                            let reqReg = parseInt(ev.requires_registration) === 1;
                            let reqFee = parseInt(ev.requires_fee) === 1;
                            $('#dash_registration').html('<span class="badge badge-success">Required</span>');
                            $('#dash_fee').html(reqFee ? `$${parseFloat(ev.registration_fee || 0).toFixed(2)}` : 'Free');
                            $('#dash_registration_section').toggle(reqReg);
                            ...
                            // Actuals Ledger (recorded transactions)
                            let $actualsBody = $('#dash_actuals_table tbody').empty();
                            if (data.actuals && data.actuals.length > 0) {
                                $.each(data.actuals, function(i, act) {
                                    let badgeClass = act.entry_type === 'Expense' ? 'badge-secondary' : 'badge-success';
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge ${badgeClass}">${escapeHtml(act.entry_type)}</span></td>
                                        <td>$${parseFloat(act.amount).toFixed(2)}</td>
                                    </tr>
                                `);
                                });
                            } else {
                                $actualsBody.append('<tr><td colspan="3" style="text-align:center; color:#64748b;">No actual transactions recorded.</td></tr>');
                            }

                            // Attendance sections only shown/loaded when registration is required
                            $('#dash_attendance_summary').toggle(reqReg);
                            $('#dash_roster_card').toggle(reqReg);

                            if (reqReg) loadDashboardRoster(id);
...
<body>
    ...
    <main style="max-width: 1200px; margin: 20px auto; padding: 0 15px;">
        ...
        <div class="card">
            <h2 style="margin-top:0; color:#28089a;">Event Information</h2>
        ...
        <div id="dashboardContent" class="dashboard-container" style="display: none;">
            ...
            <div>
                <div class="card">
                    ...
                    <div class="detail-section" id="dash_registration_section">
            ...
            <div>
                ...
                <div class="card" id="dash_attendance_summary" style="margin-bottom: 20px;">
                ...
                <div class="card" style="margin-bottom: 20px;">
                    <h2 style="margin-top:0; color:#28089a;">Actual Transactions</h2>
                    <table class="data-table" id="dash_actuals_table">
                        <thead>
                            <tr>
                                <th>Description</th>
                                <th>Type</th>
                ...
                <div class="card" id="dash_roster_card">
