...
try {
    switch ($action) {
        ...
        // --- FETCH CONTACTS LOOKUP WITH PHONE & EMAIL ---
        case 'get_contacts_list':
            $sql = "SELECT contact_id, 
                           CONCAT(COALESCE(first_name, ''), ' ', COALESCE(last_name, '')) AS full_name,
                           COALESCE(phone_1, phone, '') AS phone,
                           COALESCE(c_email, email, '') AS email 
                    FROM contacts 
                    ORDER BY first_name ASC, last_name ASC";
            $result = $db->query($sql);
            if (!$result) {
                // Fallback if schema uses alternative field names
                $sqlFallback = "SELECT contact_id, 
                                       CONCAT(COALESCE(first_name, ''), ' ', COALESCE(last_name, '')) AS full_name 
                                FROM contacts 
                                ORDER BY first_name ASC, last_name ASC";
                $result = $db->query($sqlFallback);
