...
<head>
  ...
  <script>
    $(document).ready(function() {
        ...
        // Preload reference lists for accurate label resolution
        function loadReferenceData() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success) {
                        ministriesCache = data.ministries || [];
                    }
                }
            });
        }
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success && data.prgevnt) {
                        ...
                        if (ev.contact_id) {
                            let foundContact = contactsCache.find(c => String(c.contact_id) === String(ev.contact_id));
                            if (foundContact) contactName = foundContact.full_name;
                        }
                        ...
                        if (contactDetails.length > 0) {
                            contactDisplay += `<br><span style="color:#475569; font-size:0.9rem;">${contactDetails.join(' &nbsp;|&nbsp; ')}</span>`;
                        }
                        $('#dash_contact').html(contactDisplay);

                        // Location, Purpose, Deliverables/Goals, Notes
                        ...
                        if (hasDocument) {
                            ...
                            $('#dash_document_link').attr('href', downloadUrl).text(`📄 ${ev.document_name}`);
                            $('#btnDownloadPacket').attr('href', downloadUrl).show();
                        } else {
...
<body>
  ...
  <main style="max-width: 1200px; margin: 20px auto; padding: 0 15px;">
      ...
      <div id="dashboardPlaceholder" class="card" style="margin-top: 20px; text-align: center; color: #64748b; padding: 3rem 1rem;">
          <p style="font-size: 1.1rem; margin:0;">Please select an event above to view its operational details, schedule, collaborators, budget ledger, and attendance.</p>
      </div>

      <!-- Dashboard Grid -->
      <div id="dashboardContent" class="dashboard-container" style="display: none;">
          
          <!-- Left Column: Comprehensive Event Details -->
          <div>
              <div class="card">
                  ...
                  <div class="detail-section">
                      <div class="detail-label">Ministry Support & Collaboration</div>
                  ...
                  <div class="detail-section">
                      <div class="detail-label">Special Requests and Specific Support Notes</div>
