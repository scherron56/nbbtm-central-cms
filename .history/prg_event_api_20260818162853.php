    ...
    switch ($action) {
        ...
        case 'save_event':
            ...
            if (!empty($_FILES['event_document']['name']) && $_FILES['event_document']['error'] === UPLOAD_ERR_OK) {
                ...
                $origName = basename($_FILES['event_document']['name']);
                $ext = strtolower(pathinfo($origName, PATHINFO_EXTENSION));

                if (in_array($ext, ['pdf', 'doc', 'docx'])) {
                    $safeFileName = 'event_' . uniqid() . '.' . $ext;
                    $targetPath = $uploadDir . $safeFileName;

                    if (move_uploaded_file($_FILES['event_document']['tmp_name'], $targetPath)) {
                        $uploadedPath = 'uploads/documents/' . $safeFileName;
                        $uploadedName = $origName;
                    }
                }
            }
            ...
            if (empty($id)) {
                $stmt = $db->prepare("INSERT INTO programs_events (prg_evnt_name, min_comm_id, location, goal, requires_registration, requires_fee, registration_fee, notes, document_path, document_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sisiiidsss", $prgevntname, $min_comm_id, $location, $goal, $reqReg, $reqFee, $fee, $notes, $uploadedPath, $uploadedName);
                $stmt->execute();
                $id = $db->insert_id;
                $stmt->close();
            } else {
                if ($uploadedPath !== null) {
                    $stmt = $db->prepare("UPDATE programs_events SET prg_evnt_name=?, min_comm_id=?, location=?, goal=?, requires_registration=?, requires_fee=?, registration_fee=?, notes=?, document_path=?, document_name=? WHERE prg_evnt_id=?");
                    $stmt->bind_param("sisiiidsssi", $prgevntname, $min_comm_id, $location, $goal, $reqReg, $reqFee, $fee, $notes, $uploadedPath, $uploadedName, $id);
                } else {
        ...
        case 'delete_event':
            ...
            // Delete associated document from disk if exists
            $fileStmt = $db->prepare("SELECT document_path FROM programs_events WHERE prg_evnt_id = ?");
            $fileStmt->bind_param("i", $id);
            $fileStmt->execute();
            $row = $fileStmt->get_result()->fetch_assoc();
            $fileStmt->close();

            if ($row && !empty($row['document_path']) && file_exists(__DIR__ . '/' . $row['document_path'])) {
                unlink(__DIR__ . '/' . $row['document_path']);
            }
