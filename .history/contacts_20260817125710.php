...
$(document).ready(function() {
  ...
  $('#zipcode').on('input', function() {
    ...
    if (zip.length === 5 && /^\d{5}$/.test(zip)) {
      $.getJSON('lookupZip.php', { zip: zip }, function(response) {
        if (response.found) {
          $('#city').val(response.data.city);
          $('#state').val(response.data.state_code);
        }
      });
    }
  });
  ...
  $('#city').on('input', function() {
    ...
    if (query.length >= 3) {
      cityTimer = setTimeout(function() {
        $.getJSON('lookupZip.php', { city: query }, function(response) {
          let $datalist = $('#city-list').empty();
          if (response.found) {
            $.each(response.data, function(index, item) {
              let optText = item.city + ', ' + item.state_code + ' (' + item.zip_code + ')';
              let $option = $('<option>').val(item.city).attr('label', optText);
              $option.data('state', item.state_code);
              $option.data('zip', item.zip_code);
  ...
  $('#city').on('change', function() {
    ...
    let $match = $('#city-list option').filter(function() {
      return $(this).val() === selectedVal;
    }).first();

    if ($match.length) {
      if ($match.data('state')) $('#state').val($match.data('state'));
      if ($match.data('zip')) $('#zipcode').val($match.data('zip'));
    }
  });
    ...
    <form id="contact-form" name="contact-form">
      ...
      <fieldset class="form-grid-section-9">
        ...
        <div class="field-group" style="--colspan: 3; --rowspan: 1;">
          ...
          <input type="text" id="city" name="city" list="city-list" autocomplete="off" />
