...
try {
    switch ($action) {
        ...
        case 'save_event':
            ...
            $id = !empty($_POST['prg_evnt_id']) ? (int)$_POST['prg_evnt_id'] : null;
            $prgevntname = trim($_POST['prg_evnt_name'] ?? '');
            $min_comm_id = (int)($_POST['min_comm_id'] ?? 0);
            ...
            if (empty($id)) {
                ...
                // Clear old schedules, budgets, and collaborations
        ...
        case 'delete_event':
            ...
            $stmt2 = $db->prepare("DELETE FROM programs_events WHERE prg_evnt_id = ?");
        ...
        case 'toggle_attendance':
            ...
            $reqFee = (int)($evRes['requires_fee'] ?? 0) === 1;
            ...
            if (!$reqFee) {
                ...
                if ((int)$regRes['is_completed'] === 1 || in_array($statusLower, ['paid', 'completed', 'waived', 'n/a'])) {
                    $isDone = true;
                }
            }
