...
$(document).ready(function() {
  ...
  // Fetch classes via API for selected session with Teacher Name
  function loadClasses(sessionId) {
    const previousSelected = $('#classSelect').val();
      ...
      success: function(res) {
        if (res.success && Array.isArray(res.data)) {
          res.data.forEach(cls => {
            const teacherText = cls.teacher_name ? ` — Teacher: ${cls.teacher_name}` : '';
            $classSelect.append($('<option>', {
              value: cls.vbs_class_id,
              text: `${cls.vbs_class_desc}${teacherText}`
            }));
          });

          if (previousSelected) {
            $classSelect.val(previousSelected);
          }
        }
      }
    });
  ...
  function renderRosterTable(students) {
    ...
    // Group students by class and capture teacher name
    const grouped = {};
    students.forEach(student => {
      ...
      if (!grouped[classId]) {
        grouped[classId] = { name: className, teacher: teacherName, students: [] };
      }
    ...
    Object.keys(grouped).forEach(classId => {
      ...
      const teacherDisplay = group.teacher ? `<span class="class-teacher-label">| &nbsp; Teacher: <strong>${escapeHtml(group.teacher)}</strong></span>` : '';

      tbody.append(`
        <tr class="class-group-row" data-class-id="${classId}">
          <td colspan="4">🏫 ${escapeHtml(group.name)} ${teacherDisplay}</td>
        </tr>
      `);
