...
$(document).ready(function() {
  ...
  // Class filter dropdown change handler
  $('#classSelect').on('change', function() {
    const selectedClass = $(this).val();
    
    // Filter student list to match selected class
    let filteredStudents = currentStudents;
    if (selectedClass) {
      filteredStudents = currentStudents.filter(s => {
        const cName = s.vbs_class_desc || ('Class #' + s.class_id);
  ...
  function applyFilters() {
    ...
    $('#studentTable tbody tr').each(function() {
      ...
      if ($row.hasClass('class-group-row')) {
        const rowClass = $row.data('class-name');
        if (selectedClass && rowClass !== selectedClass) {
          $row.hide();
        } else {
          // If filtering by specific student, hide header if no matching student belongs to it
          if (selectedContactId) {
            const hasVisibleChild = $(`#studentTable tbody tr[data-class-name="${rowClass}"][data-id="${selectedContactId}"]`).length > 0;
    ...
    $.ajax({
      ...
      success: function(res) {
        if (res.success) {
          ...
          populateClassDropdown(currentStudents);
  ...
  // Populate dynamic class dropdown
  ...
  function renderRosterTable(students) {
    ...
    Object.keys(grouped).forEach(className => {
      tbody.append(`
        <tr class="class-group-row" data-class-name="${escapeHtml(className)}">
      ...
      grouped[className].forEach(student => {
        ...
        const rowHtml = `
          <tr data-id="${student.contact_id}" data-class-name="${escapeHtml(className)}" data-session-id="${student.vbs_sessions_id}">
