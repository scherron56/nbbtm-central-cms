...
$userRole    = $_SESSION['user_role'] ?? 'browse';
?>
<header class="site-header">
  ...
  <nav class="navbar"> 
    <ul class="nav-links"> 
      ...
      <li class="dropdown">
        <a href="#" class="<?= in_array($currentPage, ['events.php', 'event_dashboard.php', 'event_registration.php', 'event_checkin.php']) ? 'active' : '' ?>">Programs / Events</a>
        <ul class="submenu">
      ...
      <!-- Admin Utilities Dropdown (Visible only to authenticated Admins) -->
      <?php if (isAdmin()): ?>
        <li class="dropdown">
          <a href="#" class="<?= in_array($currentPage, ['admin_mailer.php', 'user_management.php']) ? 'active' : '' ?>" style="border-left: 2px solid #f59e0b;">
            Admin Utilities &#9662;
          </a>
          <ul class="submenu">
            <li><a href="admin_mailer.php" class="<?= ($currentPage === 'admin_mailer.php') ? 'active' : '' ?>">✉️ Send Email(s) - Contacts</a></li>
            <li><a href="ministry_mailer.php" class="<?= ($currentPage === 'ministry_mailer.php') ? 'active' : '' ?>">✉️ Send Email(s) - Ministry & Committee</a></li>
      ...
      <?php else: ?>
        <li style="margin-left: auto; display: flex; align-items: center; gap: 10px;">
          <span style="font-size: 0.85rem; opacity: 0.9; font-style: normal;">Mode: <strong>Browse</strong></span>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (openLoginBtn) openLoginBtn.addEventListener('click', (e) => { e.preventDefault(); loginModal.classList.remove('hidden'); });
  if (closeLoginBtn) closeLoginBtn.addEventListener('click', () => { loginModal.classList.add('hidden'); loginErrorMsg.classList.add('hidden'); });

  if (openChangePwdBtn) openChangePwdBtn.addEventListener('click', (e) => { e.preventDefault(); changePwdModal.classList.remove('hidden'); });
  if (closeChangePwdBtn) closeChangePwdBtn.addEventListener('click', () => { changePwdModal.classList.add('hidden'); changePwdMsg.classList.add('hidden'); });
  ...
  // Handle Password Change Submission via AJAX
  if (changePwdForm) {
