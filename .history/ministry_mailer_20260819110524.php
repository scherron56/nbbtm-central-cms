...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $selectedEmails = $_POST['selected_emails'] ?? []; // Array of recipient JSON objects or raw emails
    ...
    if (!empty($selectedEmails) && is_array($selectedEmails)) {
        foreach ($selectedEmails as $rawItem) {
            ...
            if ($parsed && !empty($parsed['email'])) {
                $recipientsToSend[] = [
                    'name'  => $parsed['name'] ?: 'Ministry Member',
                    'email' => $parsed['email']
                ];
            }
        }
...
<head>
  ...
  <title>Ministry Email Dispatcher - NBBTM CMS</title>
...
<main class="dashboard-container">
  ...
  <div class="card" style="max-width: 950px; margin: 0 auto 2rem auto;">
    <h3>✉️ Ministry & Committee Email Dispatcher</h3>
...
$(document).ready(function() {
        ...
        if (res.status === 'success' && res.data.length > 0) {
          ...
          $.each(res.data, function(i, m) {
            ...
            if (m.c_email && m.c_email.trim() !== '') {
              ...
              // Display ONLY the full name in the dropdown option
              options += `<option value="${escapeHtml(m.c_email)}" data-name="${escapeHtml(fullName)}" data-email="${escapeHtml(m.c_email)}">${escapeHtml(fullName)}</option>`;
              countValid++;
            }
          });
  ...
  function syncSelectedRecipients() {
    ...
    selectedOptions.each(function() {
      ...
      const chip = $('<span class="recipient-chip"></span>').text(`${name} <${email}>`);
      displayBox.append(chip);

      // Create hidden inputs for PHP POST processing
      const payload = JSON.stringify({ name: name, email: email });
      $('<input>').attr({
        type: 'hidden',
