    ...
    $(document).ready(function() {
                ...
                success: function(data) {
                    if(data.success) {
                        ...
                        $.each(contactsCache, function(i, c) {
                            let label = escapeHtml(c.full_name);
                            if (c.phone || c.email) {
                                let details = [c.phone, c.email].filter(Boolean).join(' | ');
                                label += ` (${escapeHtml(details)})`;
                            }
                            $cSelect.append(`<option value="${c.contact_id}">${label}</option>`);
                        });
                    }
                }
            });
        ...
        $('#contact_id').on('change', function() {
            ...
            if(cid) {
                ...
                $('#signoff_contact_label').text(`Signed by Primary Contact (${contactName.split('(')[0].trim()})`);
            } else {
                $('#signoff_contact_label').text(`Signed by Primary Contact`);
            }
        });

        // 2. Load Sponsoring Ministries and Support Ministries (5000, 5002, 5007, 5008, 5020, 5018)
