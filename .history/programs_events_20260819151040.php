...
<head>
  ...
  <script>
    $(document).ready(function() {
        ...
        // Building maintenance options toggle & Dynamic Sign-off Sync
                ...
                success: function(data) {
                    if(data.success) {
                        ...
                        $.each(contactsCache, function(i, c) {
                            $cSelect.append(`<option value="${c.contact_id}">${escapeHtml(c.full_name)}</option>`);
                        });
                    }
                }
            });
        ...
        $('#contact_id').on('change', function() {
            let cid = $(this).val();
            let contactName = $('#contact_id option:selected').text();
            if(cid) {
                let found = contactsCache.find(c => c.contact_id == cid);
                if(found) {
                    if(found.phone) $('#contact_phone').val(found.phone);
                    if(found.email) $('#contact_email').val(found.email);
                }
                $('#signoff_contact_label').text(`Signed by Primary Contact (${contactName})`);
            } else {
                $('#signoff_contact_label').text(`Signed by Primary Contact`);
            }
        });
                ...
                success: function(data) {
                    if(data.success) {
                        ...
                        $.each(data.ministries, function(i, m) {
                            $minSelect.append(`<option value="${m.min_comm_id}">${escapeHtml(m.min_comm_name)}</option>`);

                            let isTarget = supportMinistryNames.some(name => m.min_comm_name.toLowerCase().trim() === name.toLowerCase().trim());
                            if(isTarget) {
                                let isAdmin = m.min_comm_name.toLowerCase().includes('administration');
                                let isMaint = m.min_comm_name.toLowerCase().includes('building maintenance');

                                let itemHtml = `
                                    <div class="support-min-item">
                                        <label style="font-weight: 600; cursor:pointer;">
                                            <input type="checkbox" name="support_min_ids[]" value="${m.min_comm_id}" data-name="${escapeHtml(m.min_comm_name)}" class="chk-support-min" 
                                ...
                                if(isAdmin) {
                                    itemHtml += `<input type="hidden" name="support_min_ids[]" value="${m.min_comm_id}">`;
                                }

                                if(isMaint) {
                                    itemHtml += `
                                        <div id="maintOptions" class="support-radio-options" style="display:none;">
                                            <label><input type="radio" name="support_option_${m.min_comm_id}" value="Option A" checked> Option A</label>
                                            <label><input type="radio" name="support_option_${m.min_comm_id}" value="Option B"> Option B</label>
                                        </div>
                                    `;
                                }
        ...
        // Dynamically rebuild the Affected Ministry sign-off items based on selections
        function refreshSignoffCheckboxes() {
            ...
            $('.chk-support-min:checked').each(function() {
                ...
                let minId = $(this).val();
                if (minName && !minName.toLowerCase().includes('administration')) {
                    $container.append(`
                ...
                <fieldset class="dashboard-main-grid fieldset-relative">
                    ...
                    <div class="field-group" id="acknowledgementSection" style="--colspan: 2;">
                        ...
                        <div class="auth-box">
                            ...
                            <!-- Administration Support Sign-off -->
                            <label class="auth-item">
                            ...
                            <!-- Affected Ministries (Dynamically Populated) -->
