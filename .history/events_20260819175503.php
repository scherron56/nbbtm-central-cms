    ...
    $(document).ready(function() {
        ...
        function showStatusMessage(message, type = 'success') {
            ...
            $box.removeClass('alert-success alert-error')
                ...
                .html(message).stop(true, true).fadeIn(200);
            if (type === 'success') setTimeout(() => $box.fadeOut(500), 5000);
        }
        ...
        function loadEventCategories() {
            $.ajax({
                ...
                type: 'GET',
                data: { action: 'get_categories', entity_type: 'event', only_active: 1 },
                dataType: 'json',
        ...
        function populateCategoryDropdowns() {
            $('.attach-cat-select').each(function() {
                ...
                $.each(eventCategoriesCache, function(i, c) {
        ...
        function calculateLiveBudgetSummary() {
            ...
            let $bc = $('#balanceCard');
            if (netBalance < 0) $bc.css({'background': '#fef2f2', 'border-color': '#fee2e2', 'color': '#ef4444'});
            else if (netBalance > 0) $bc.css({'background': '#f0fdf4', 'border-color': '#dcfce7', 'color': '#10b981'});
                ...
                success: function(data) {
                    if(data.success && data.contacts) {
                        ...
                        $.each(contactsCache, function(i, c) {
                            $cSelect.append(`<option value="${c.contact_id}">${escapeHtml(c.full_name || `Contact #${c.contact_id}`)}</option>`);
                        });
        ...
        $('#contact_id').on('change', function() {
            let cid = $(this).val();
            let selectedName = $('#contact_id option:selected').text().trim();
            if(!cid) {
                $('#contact_phone').val('');
                $('#contact_email').val('');
            ...
            $('#signoff_contact_label').html(`<strong>Signed by Primary Contact (${escapeHtml(selectedName)})</strong>`);
        });
        ...
        function loadMinistries() {
            $.ajax({
                ...
                success: function(data) {
                    if(data.success) {
                        let $minSelect = $('#min_comm_id').empty().append('<option value="">-- Select Sponsoring Ministry --</option>');
        ...
        function refreshSignoffCheckboxes() {
            let $container = $('#dynamicMinistrySignoffs');
            let checkedIds = [];
            $container.find('.chk-signoff:checked').each(function() {
                let nameAttr = $(this).attr('name') || '';
                let idMatch = nameAttr.match(/signoff_min_(\d+)/);
                if (idMatch) checkedIds.push(parseInt(idMatch[1]));
            });
            $container.empty();
            ...
            $('.chk-support-min:checked').each(function() {
                ...
                if (minName && minId !== ADMIN_SUPPORT_ID) {
                    let isChecked = checkedIds.includes(minId) ? 'checked' : '';
                    $container.append(`
                        <label class="auth-item">
                            <input type="checkbox" name="signoff_min_${minId}" class="chk-signoff" ${isChecked}>
                            <span><strong>Signed by Affected Ministry:</strong> ${escapeHtml(minName)}</span>
                        </label>
                    `);
                }
            });
        ...
        function renderAttachmentsList(attachments) {
            ...
            if (attachments.length === 0) {
                ...
                $.each(attachments, function(i, att) {
                    ...
                    $tbody.append(`
                        <tr>
                            <td><span class="badge badge-secondary">${escapeHtml(att.category_name || 'General')}</span></td>
        ...
        $('#btnDelete').on('click', function() {
            if (!CAN_EDIT) return;
            clearStatusMessage();
            if(confirm("Are you sure you want to delete this event? All associated schedules, registrations, support assignments, budgets, and attachments will be permanently deleted.")) {
                $.ajax({
                    url: 'prg_event_api.php',
                    type: 'POST',
            ...
            <form id="eventForm" enctype="multipart/form-data">
                ...
                <fieldset class="dashboard-main-grid fieldset-relative">
                    ...
                    <div class="field-group" style="--colspan: 2;">
                        ...
                        <div id="ministrySupportContainer" class="support-min-grid"></div>
                    </div>
                    ...
                    <div class="field-group" style="--colspan: 2;">
                        <label><strong>Event Documents & Packets:</strong></label>
                        <div class="doc-management-box">
                            <label style="font-size: 0.85rem; color: #475569; font-weight: bold; margin-bottom: 5px; display: block;">Attached Documents:</label>
                            <div id="noAttachmentsMsg" style="color: #64748b; font-style: italic; margin-bottom: 8px;">No documents attached to this event yet.</div>
                            ...
                            <label style="font-size: 0.85rem; color: #475569; font-weight: bold; margin-bottom: 5px; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <button type="button" id="btnAddFileRow" class="btn btn-sm btn-success" style="padding: 2px 8px; font-size: 0.8rem;">+ Add Another File</button>
                            </label>
                    ...
                    <div class="field-group" id="acknowledgementSection" style="--colspan: 2;">
                        <label><strong>Acknowledgement & Authorization Sign-Offs (Optional):</strong></label>
                        <div class="auth-box">
                            <label class="auth-item">
                                <input type="checkbox" id="signoff_contact" name="signoff_contact" class="chk-signoff">
                                <span id="signoff_contact_label"><strong>Signed by Primary Contact</strong></span>
                            </label>

                            <label class="auth-item">
                                <input type="checkbox" id="signoff_admin" name="signoff_admin" class="chk-signoff">
                            ...
                            <label class="auth-item">
                                <input type="checkbox" id="signoff_pastor" name="signoff_pastor" class="chk-signoff">
