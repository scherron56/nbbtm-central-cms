...
// 1. Fetch cities for all allowed states (or a single requested state)
if ($action === 'getCities') {
    $requestedState = trim($_GET['state'] ?? '');

    if (!empty($requestedState)) {
        // Specific single state
        $stmt = $db->prepare("SELECT DISTINCT city, state_code FROM us_zip_codes WHERE state_code = ? ORDER BY state_code ASC, city ASC");
        $stmt->bind_param("s", $requestedState);
    } else {
        // All configured default states
...
// 3. Reverse Lookup: Fetch City & State from a 5-digit ZIP
