...
// Handle Form Submissions (Create, Update, Delete, Toggle Status)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                if (!empty($password)) {
                    if (strlen($password) >= 8) {
                        $passwordHash = password_hash($password, PASSWORD_DEFAULT);
                        $stmt = $db->prepare("UPDATE system_users SET username = ?, email = ?, role = ?, password_hash = ? WHERE user_id = ?");
                    ...
                    $stmt = $db->prepare("UPDATE system_users SET username = ?, email = ?, role = ? WHERE user_id = ?");
    ...
    if ($action === 'delete_user') {
        ...
        if ($userId === (int)$_SESSION['user_id']) {
            ...
            $stmt = $db->prepare("DELETE FROM system_users WHERE user_id = ?");
    ...
    <form method="POST" id="user-form" style="max-width: 100%; box-shadow: none; padding: 0;">
      ...
      <div class="form-grid-section-12">
        <div class="field-group" style="--colspan: 3;">
          ...
          <input type="text" id="username" name="username" required style="padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
        </div>

        <div class="field-group" style="--colspan: 3;">
          <label for="email"><strong>Email Address</strong></label>
          <input type="email" id="email" name="email" required style="padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
        </div>

        <div class="field-group" style="--colspan: 3;">
          <label for="password"><strong id="password-label">Password</strong></label>
          <input type="password" id="password" name="password" minlength="8" required placeholder="Min. 8 characters" style="padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px;">
        </div>

        <div class="field-group" style="--colspan: 3;">
          <label for="role"><strong>Role</strong></label>
          <select id="role" name="role" style="padding: 8px; border: 1px solid #cbd5e1; border-radius: 4px; height: 38px;">
...
function editUser(user) {
  ...
  // Make password optional for existing user updates
